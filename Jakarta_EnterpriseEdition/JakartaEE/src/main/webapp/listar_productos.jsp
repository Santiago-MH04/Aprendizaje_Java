    ...
    <table class="table table-hover table-striped">
        ...
        <c:forEach items="${requestScope.productos}" var="P">
            <tr>
                ...
                <c:if test="${requestScope.username.present}">
                    <td>${P.getPrecio()}</td>
                    <td><a class="btn btn-sm btn-primary" href="<%=request.getContextPath()%>/carrito/modificar-session?id=${P.ID}">Añadir al carrito</a></td>
                    <td><a class="btn btn-sm btn-success" href="<%=request.getContextPath()%>/productos/form?id=${P.ID}">Modificar</a></td>
                    <td><a class="btn btn-sm btn-danger" onclick="return confirm('¿Está seguro que desea eliminar este producto?')"
                            href="<%=request.getContextPath()%>/productos/eliminar?id=${P.ID}">
                            Eliminar
                        </a>
                    </td>
                </c:if>
